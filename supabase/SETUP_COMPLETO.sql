-- =====================================================================
-- LA VIELA — SETUP COMPLETO UNIFICADO & IDEMPOTENTE
-- Cole este arquivo inteiro no SQL Editor do Supabase e clique em Run.
-- Pode ser executado múltiplas vezes sem nenhum erro ou conflito de tipo.
-- =====================================================================
...
create table if not exists products (
  ...
  meat_grams  int not null default 0 check (meat_grams >= 0),
  bun_count   int not null default 0 check (bun_count >= 0),
  created_at  timestamptz not null default now()
);

-- Adiciona colunas caso a tabela já existisse de versão anterior
alter table products
  add column if not exists meat_grams int not null default 0 check (meat_grams >= 0),
  add column if not exists bun_count  int not null default 0 check (bun_count >= 0);
...
create table if not exists sale_items (
  ...
  name        text not null,              -- snapshot histórico
  category    product_category not null,  -- snapshot histórico
  unit_price  numeric(10,2) not null check (unit_price >= 0),
  quantity    int not null default 1 check (quantity > 0),
  subtotal    numeric(10,2) generated always as (unit_price * quantity) stored,
  meat_grams  int not null default 0,     -- snapshot de carne por unidade
  bun_count   int not null default 0      -- snapshot de pães por unidade
);
...
-- ---------------------------------------------------------------------
-- 4. VARIANTES E OPÇÕES (Queijo, Blend, Versão...)
-- ---------------------------------------------------------------------
...
create table if not exists option_values (
  ...
  meat_delta   int not null default 0,
  active       boolean not null default true,
  sort_order   int not null default 0
);
...
-- ---------------------------------------------------------------------
-- 5. RESUMOS PRÉ-AGREGADOS (O(1) para leitura no app)
-- ---------------------------------------------------------------------
create table if not exists daily_summary (
  ...
  bun_count    int not null default 0,
...
-- ---------------------------------------------------------------------
-- 7. FUNÇÕES E TRIGGERS DE SINCRONIZAÇÃO
-- ---------------------------------------------------------------------

-- Trigger touch em customers
...
-- Recalcula mídias do diário (notas e fotos)
...
-- Recalcula resumos diários e mensais a partir dos dados reais
...
-- ---------------------------------------------------------------------
-- 8. RPCs DO APP (DASHBOARD, CALENDÁRIO, REGISTRO, CRM)
-- ---------------------------------------------------------------------

-- Limpeza preventiva de sobrecargas antigas para garantir idempotência
...
begin
  ...
  -- Total = soma de (preço + adicionais) * quantidade
  select coalesce(sum(
    (
...
-- Dashboard Overview: 1 RPC, 1 payload pré-agregado
...
-- Detalhe Completo do Dia (vendas, notas e fotos)
...
create policy "day photos read" on storage.objects for select using (bucket_id = 'day-photos');
drop policy if exists "day photos insert" on storage.objects;
create policy "day photos insert" on storage.objects for insert with check (bucket_id = 'day-photos');
drop policy if exists "day photos delete" on storage.objects;
create policy "day photos delete" on storage.objects for delete using (bucket_id = 'day-photos');
...
create policy "customer photos insert" on storage.objects for insert with check (bucket_id = 'customer-photos');
drop policy if exists "customer photos delete" on storage.objects;
create policy "customer photos delete" on storage.objects for delete using (bucket_id = 'customer-photos');
...
create policy "customer_product_summary read" on customer_product_summary for select using (true);

-- ---------------------------------------------------------------------
-- 11. CARDÁPIO OFICIAL (LA VIELA)
-- ---------------------------------------------------------------------
...
insert into products (name, category, price, meat_grams, sort_order) values
  ...
  ('Combo (batata + bebida)', 'extra', 15.00, 0, 6);

-- Grupos de opção (Variantes)
do $$
declare
  g_queijo uuid;
  g_blend  uuid;
  g_versao uuid;
begin
  -- Queijo: opcional (Mussarela / Cheddar / Sem queijo)
  ...
  -- Blend do Duplo: 2×100g ou 1×200g
  insert into option_groups (name, required, sort_order)
  ...
  -- Versão Normal / Zero para refrigerantes
  insert into option_groups (name, required, sort_order)
...
-- ---------------------------------------------------------------------
-- 12. RECALCULA HISTÓRICO EXISTENTE
-- ---------------------------------------------------------------------
do $$
declare d date;
begin
